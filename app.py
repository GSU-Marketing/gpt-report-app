...
# --- PAGE 2: Geography & Program ---
    ...
    if "Zip Code" in filtered_df.columns:
        ...
        fig = px.treemap(zip_counts.head(50), path=["Zip Code"], values="Count",
                         title="Lead Volume by ZIP Code")
        st.plotly_chart(fig, config={'displayModeBar': False})
...
# --- PAGE 3: Engagement & Traffic ---
    ...
    if "Ping UTM Medium" in filtered_df.columns:
        fig = px.bar(filtered_df[filtered_df['Ping UTM Medium'].notna()], x="Ping UTM Medium",
                     title="Traffic by UTM Medium")
        st.plotly_chart(fig, config={'displayModeBar': False})

    if "Ping UTM Campaign" in filtered_df.columns:
        fig = px.bar(filtered_df[filtered_df['Ping UTM Campaign'].notna()].value_counts().reset_index().rename(columns={0: "Count"}),
